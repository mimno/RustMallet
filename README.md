...
The package includes Python bindings built with [PyO3](https://pyo3.rs) and [maturin](https://maturin.rs), published on PyPI as [`pyrmallet`](https://pypi.org/project/pyrmallet/). There are two layers: a sklearn-compatible `LatentDirichletAllocation` class and a lower-level `_rust_mallet` extension module.

```bash
pip install pyrmallet
```

See [pyrmallet/README.md](pyrmallet/README.md) for the API and examples, or build from source:
