...
pub fn run_iteration<R: Rng>(model: &mut TopicModel, corpus: &Corpus, rng: &mut R) {
    // Copy scalar fields before taking field-level borrows.
    let _alpha_sum = model.alpha_sum; // retained for symmetry; not used in sampling
    let beta       = model.beta;
    let beta_sum   = model.beta_sum;
    let topic_mask = model.topic_mask;
    let topic_bits = model.topic_bits;
    let num_topics = model.num_topics;

    // Pre-compute using model fields before splitting the borrows.
    let mut smoothing_only_mass = 0.0f64;
    let mut cached_coefficients = vec![0.0f64; num_topics];
    for t in 0..num_topics {
        let denom = model.tokens_per_topic[t] as f64 + beta_sum;
        smoothing_only_mass += model.alpha[t] * beta / denom;
        cached_coefficients[t] = model.alpha[t] / denom;
    }
    ...
    // Field-splitting borrows: Rust allows simultaneous mutable borrows of
    // non-overlapping struct fields. Passing them as separate parameters to
    // sample_doc lets the borrow checker see that type_topic_counts[word_id]
    // and tokens_per_topic do not alias, so we can hold a reference to the
    // inner vec while still writing to tokens_per_topic.
    let type_topic_counts = &mut model.type_topic_counts;
    let tokens_per_topic  = &mut model.tokens_per_topic;
    let doc_topics        = &mut model.doc_topics;
    let alpha             = model.alpha.as_slice();

    for doc_idx in 0..corpus.docs.len() {
        sample_doc(
            type_topic_counts,
            tokens_per_topic,
            doc_topics,
...
fn sample_doc<R: Rng>(
    type_topic_counts: &mut Vec<Vec<u32>>,
    tokens_per_topic:  &mut Vec<u32>,
    doc_topics:        &mut Vec<Vec<u32>>,
    alpha:             &[f64],
    beta:              f64,
    beta_sum:          f64,
    topic_mask:        u32,
